...
describe('Tea Util', function () {
  ...
  it('toJSONString should ok', function () {
    ...
    assert.deepStrictEqual(
      Client.toJSONString({ str: 'test', number: 1, bool: false, null: null }),
      '{"str":"test","number":1,"bool":false,"null":null}'
    );
    assert.deepStrictEqual(Client.toJSONString([]), '[]');
  ...
  it('stringifyMapValue', function () {
    ...
    assert.deepStrictEqual(Client.stringifyMapValue({ number: 100 }), {
      number: '100',
    });
    assert.deepStrictEqual(Client.stringifyMapValue({ bool: true }), {
      bool: 'true',
    });
    assert.deepStrictEqual(
      Client.stringifyMapValue({
        bool: true,
        string: 'str',
        number: '0',
        null: null,
  ...
  it('anyifyMapValue', function () {
    ...
    assert.deepStrictEqual(Client.anyifyMapValue({ number: '100' }), {
      number: '100',
    });
    assert.deepStrictEqual(Client.anyifyMapValue({ string: 'string' }), {
      string: 'string',
    });
  ...
  it('assertAsBytes', function () {
    assert.deepStrictEqual(
      Client.assertAsBytes(Buffer.from('test')),
      Buffer.from('test')
    );
    assert.throws(() => {
  ...
  it('getUserAgent', function () {
    assert.strictEqual(
      Client.getUserAgent(''),
      `AlibabaCloud (${platform()}; ${arch()}) Node.js/${
        process.version
      } Core/1.0.1 TeaDSL/1`
    );
    assert.strictEqual(
      Client.getUserAgent('2019'),
      `AlibabaCloud (${platform()}; ${arch()}) Node.js/${
        process.version
      } Core/1.0.1 TeaDSL/1 2019`
    );
  });
